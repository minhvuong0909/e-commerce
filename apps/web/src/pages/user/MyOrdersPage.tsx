...
import type { OrderUI, Status } from '../../models/OrderRequests'
    ...
    <div className='mx-auto max-w-7xl px-4 py-8 md:px-6'>
      ...
      <div className='min-h-[320px]'>
        {isLoading ? (
          ...
          <div className='rounded-3xl border border-rose-200 bg-rose-50 px-6 py-10 text-center text-rose-900'>
            <div className='text-lg font-extrabold'>Có lỗi xảy ra</div>
            <p className='mt-2 text-sm opacity-80'>Không thể tải danh sách đơn hàng. Vui lòng thử lại.</p>
          </div>
        ) : filtered.length === 0 ? (
          <EmptyState
          ...
          <div className='space-y-4'>
            {filtered.map((order: OrderUI) => (
              <Link
                key={order.id}
                to={`/user/orders/${order.id}`}
                className='surface-card block rounded-3xl border border-slate-200 p-5 transition hover:border-slate-300'
              >
                <div className='flex flex-col gap-4 md:flex-row md:items-center md:justify-between'>
                  <div className='min-w-0'>
                    ...
                    <div className='mt-3 space-y-1 text-sm text-slate-500'>
                      ...
                      <div className='text-xs font-semibold'>
                        Phương thức thanh toán: {String(order.paymentMethod).replaceAll('_', ' ')}
                      </div>
                    </div>
                  </div>
                  <div className='flex items-end justify-between gap-4 md:block md:text-right'>
                    ...
                    <div className='inline-flex items-center gap-1 text-xs font-black text-brand-600'>
